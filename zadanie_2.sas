...
/* Transposition & SQL */
proc sort data=B_SZKODY out=B_SZKODY_sorted;
	by ID_POLISY;
run;

proc transpose data=B_SZKODY_sorted out=B_SZKODY_transposed prefix=TYP_SZKODY;
	var TYP_SZKODY;
	by ID_POLISY;
run;
proc sql;
	SELECT
		B_SZKODY_ID_POLISY_fkeys.ID_POLISY,
		B_SZKODY_transposed.TYP_SZKODY1,
		B_SZKODY_transposed.TYP_SZKODY2
	FROM (
		SELECT DISTINCT
			ID_POLISY
		FROM B_SZKODY
		) AS B_SZKODY_ID_POLISY_fkeys
	LEFT JOIN B_SZKODY_transposed
	ON B_SZKODY_ID_POLISY_fkeys.ID_POLISY = B_SZKODY_transposed.ID_POLISY
;quit;
